/* tslint:disable:max-line-length */
import { Directive, TemplateRef } from '@angular/core';
/**
 * Used for rendering the custom list item content. The template context is
 * set to the current component. To get a reference to the current text that
 * is typed by the user, use the `let-customItem` directive.
 *
 * For more examples, refer to the article on [templates]({% slug templates_multiselect %}).
 *
 * @example
 * ```ts
 * _@Component({
 * selector: 'my-app',
 * template: `
 *  <kendo-multiselect [data]="listItems" [allowCustom]="true">
 *    <ng-template kendoMultiSelectCustomItemTemplate let-customItem>
 *      <span>New Item: {{customItem}}</span>
 *    </ng-template>
 *  </kendo-multiselect>
 * `
 * })
 * class AppComponent {
 *   public listItems: Array<string> = ["Item 1", "Item 2", "Item 3", "Item 4"];
 * }
 * ```
 *
 */
export class CustomItemTemplateDirective {
    constructor(templateRef) {
        this.templateRef = templateRef;
    }
}
CustomItemTemplateDirective.decorators = [
    { type: Directive, args: [{
                selector: '[kendoMultiSelectCustomItemTemplate]'
            },] },
];
/** @nocollapse */
CustomItemTemplateDirective.ctorParameters = () => [
    { type: TemplateRef, },
];
